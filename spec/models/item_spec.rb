...
RSpec.describe Item, type: :model do
  ...
  context '保存できない場合' do
    ...
    it 'カテゴリーが空では出品できないこと' do
      ...
      @item.valid?
      expect(@item.errors.full_messages).to include("Item category can't be blank", "Item category can't be blank",
                                                    'Item category is not a number')
    end
    ...
    it '商品の状態が空では出品できないこと' do
      @item.item_status_id = ''
      @item.valid?
      expect(@item.errors.full_messages).to include("Item status can't be blank", "Item status can't be blank",
                                                    'Item status is not a number')
    end
    ...
    it '配送料の負担が空では出品できないこと' do
      ...
      @item.valid?
      expect(@item.errors.full_messages).to include("Shipping charges can't be blank", "Shipping charges can't be blank",
                                                    'Shipping charges is not a number')
    end
    ...
    it '配送元の地域がからでは出品できないこと' do
      ...
      @item.valid?
      expect(@item.errors.full_messages).to include("Shipping area can't be blank", "Shipping area can't be blank",
    ...
    it '価格が半角英数混合では出品できないこと' do
      @item.item_price = '300en'
      @item.valid?
      expect(@item.errors.full_messages).to include('Item price is not a number')
    end

    it '価格が半角英語のみでは出品できないこと' do
      @item.item_price = 'sanbyakuen'
      @item.valid?
      expect(@item.errors.full_messages).to include('Item price is not a number')
    end
